...
def replaceJoker(card_count):
    if "J" in card_count.keys() and len(card_count.keys()) > 1:
        amount_jokers = card_count["J"]
        card_count.pop("J")
        max_value = max(card_count.values())
        max_key = "2"
        for key in card_count.keys():
            if card_count[key] == max_value and card_ranks[key] > card_ranks[max_key]:
                max_key = key
        card_count[max_key] += amount_jokers

    return card_count


def getHandRank(card_count_org):
    card_count = replaceJoker(card_count_org)
